    ...
    message.channel.createOverwrite(message.guild.id, {
      ...
      message.channel.send(`**${message.author.username}** just locked the channel down for ${ms(ms(time), { long:true })}`).then(() => {
        ...
        client.lockit[message.channel.id] = setTimeout(() => {
          message.channel.createOverwrite(message.guild.id, {
            SEND_MESSAGES: null
          }).then(message.channel.send('Lockdown lifted, enjoy talking !')).catch(console.error);
          delete client.lockit[message.channel.id];
        }, ms(time));
...
exports.help = {
  ...
  usage: 'r!timedlockdown <duration>',
  example: "r!timedlockdown 20s"
};

exports.conf = {
  aliases: ['timelock', 'templock'],
  cooldown: 5
 };